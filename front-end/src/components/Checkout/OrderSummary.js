...
const OrderSummary = ({ address }) => {
    ...
    const [isOpenPopupConfirmVNPay, setIsOpenPopupConfirmVNPay] = useState(false);
    ...
    const handleCreateVNPayOnline = async () => {
        const price = cart?.totalDiscountPrice + 25000;
        await dispatch(createOrder({ jwt, dto: { paymentMethod: 'NET_BANKING', ...data } })).then(() => {
            dispatch(getNotifications(jwt));
        });
        axios.post(`http://localhost:8080/users/vnpay/${price}`).then((res) => {
            alert('thanh cong ');
        ...
        <div className={cx('wrapper')}>
            ...
            <div className={cx('payment-section')}>
                ...
                <div className={cx('payment-option')}>
                    ...
                    <div className={cx('payment-methods')}>
                        <Button
                            onClick={() => setIsOpenPopupConfirmVNPay(true)}
                            variant="contained"
                            type="submit"
                            sx={{
                                padding: '.8rem 2rem',
            ...
            {isOpenPopupConfirmVNPay && (
                <div onClick={() => setIsOpenPopupConfirmVNPay(false)} className={cx('popup-overlay')}>
                    <div onClick={(e) => e.stopPropagation()} className={cx('popup-content')}>
                        {/* Header */}
                        <div className={cx('popup-header')}>
                            <h1 className={cx('popup-title')}>Xác nhận đồng ý thanh toán online?</h1>
                            <div onClick={() => setIsOpenPopupConfirmVNPay(false)} className={cx('popup-close')}>
                                <AiOutlineCloseSquare />
                            </div>
                        </div>
                        <div className={cx('popup-footer')}>
                            <div className={cx('footer-button')}>
                                <Button
                                    onClick={() => setIsOpenPopupConfirmVNPay(false)}
                                    variant="contained"
                                    type="submit"
                                    sx={{
                                        padding: '.8rem 2rem',
                            ...
                            <div className={cx('footer-button')}>
                                <Button
                                    onClick={() => handleCreateVNPayOnline()}
                                    variant="contained"
                                    type="submit"
                                    sx={{
                                        padding: '.8rem 2rem',
