...
const OrderCard = ({ order }) => {
    ...
    // Kiểm tra nếu sản phẩm đầu tiên có số lượng <= 0
    if (order.orderItemList[0].quantity <= 0) {
        return null; // Không render gì cả
    }
                        ...
                        <p className="space-y-2 font-semibold">
                            ...
                            {order?.orderStatus === 'DELIVERED' ? (
                                <>
                                    ...
                                    />
                                    <span>Đơn hàng đã được chuyển tới</span>
                                </>
                            ) : (
                                <>
                                    <AdjustIcon sx={{ width: '15px', height: '15px' }} className={cx('icon-status')} />
                                    <span>Đơn hàng sẽ được chuyển tới trong vài ngày nữa</span>
                                </>
                            )}
                        </p>
